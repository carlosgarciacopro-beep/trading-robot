    ...
    // NEXORA v3.9 SAFE PENDING QUOTE CANDIDATE
    // Option Chain API
      ...
      try {
        ...
        if (!result || typeof result !== "object") {
          return {
            ...
            rawAvailable: false,
            errorStatus: null,
            errorMessage: "Massive respondió sin registros de quotes.",
          };
        }
    ...
    async function fetchProviderOptionChain({
      ...
      return {
        ...
        diagnostics: {
          ...
          quoteRecovery: {
            endpoint: "/v3/quotes/{optionsTicker}?order=desc&limit=1&sort=timestamp",
    ...
    function buildPendingQuoteCandidate(
      normalizedContracts = [],
      side = null,
      targetStrike = null,
      underlyingPrice = null
      ...
      const candidates = normalizedContracts
        .filter((contract) => !wantedSide || contract?.side === wantedSide)
      ...
      const sorted = [...candidates].sort((a, b) => {
        ...
        const deltaA =
          a?.delta == null
            ? 999
            : Math.abs(Math.abs(toNumber(a.delta, 0)) - deltaTarget);
        const deltaB =
          b?.delta == null
            ? 999
            : Math.abs(Math.abs(toNumber(b.delta, 0)) - deltaTarget);

        if (deltaA !== deltaB) return deltaA - deltaB;

        const volumeDiff =
          toNumber(b?.volume, 0) - toNumber(a?.volume, 0);
      ...
      return {
        status: "CANDIDATO_COTIZACION_PENDIENTE",
        executionAllowed: false,
        reason:
          "Contrato real encontrado, pero falta bid/ask válido para comprobar spread y precio de ejecución.",
        contract: {
          ...
          underlyingPrice:
            contract?.underlyingPrice ?? underlyingPrice ?? null,
          distanceFromPricePct: contract?.distanceFromPricePct ?? null,
          source: contract?.source || "massive",
        },
        missingValidation: ["bid", "ask", "spread", "execution_price"],
        instruction:
          "Confirmar bid/ask y spread en broker antes de ejecutar. Nexora NO autoriza entrada con este estado.",
      };
    }
    ...
    async function processOptionRequest({
      ...
      const pendingQuoteCandidate =
        !selection?.primary
          ? buildPendingQuoteCandidate(
              normalizedContracts,
              side,
              targetStrike,
