...
function analyzeRows(symbol, rows, mode='swing'){
  ...
  return {
    ...
    optionIdea:{
      type:side,
      strike:side==='CALL'?Math.ceil(close):side==='PUT'?Math.floor(close):null,
      expiration:mode==='intraday'?'0DTE o semanal solo con stop estricto':'7 a 14 días para swing',
      maxPremiumRisk:'Riesgo máximo sugerido: 2% a 5% de la cuenta; práctica: stop -20% a -30% de la prima',
      avoid:'Evitar si spread bid/ask está muy abierto, bajo volumen, o hay noticia fuerte sin confirmar'
    }
  };
}
